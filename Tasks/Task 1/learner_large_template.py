...
import gridworld_large_template as gt
import evaluator as ev
import time

START = (0, 0)  # start state
GOAL = (24, 49) # goal states
# Actions
RIGHT = 0
LEFT = 1
UP = 2
...
def reward(s, a, map_effect_id):
    ...
    # almost goal: [(23, 49), (25, 49), (24, 48)]
    # goal: (24, 49)
    if s == (23, 49) and a == DOWN:
        return 100
    elif s == (25, 49) and a == UP:
        return 100
    elif s == (24, 48) and a == RIGHT:
        return 100
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.QUIT:
                ...
                for state in range(len(Trajectory) + 1):
                    ...
                    # break when (24, 49)
                    if Trajectory[state] == (24,49):
                        break

                    if gt.matrix[Trajectory[state][0], Trajectory[state][1]] == 4:

                        agent.x = gt.WIDTH // 80 + gt.WIDTH // 160 - gt.CENTER - agent.w // 2 + start_x * gt.cellSize
                        agent.y = gt.WIDTH // 80 + gt.WIDTH // 160 - gt.CENTER - agent.h // 2 + start_y * gt.cellSize

                    else:

                        if Trajectory[state][0] > Trajectory[state + 1][0] or Trajectory[state][0] < \
                                Trajectory[state + 1][0]:
                        ...
                        elif Trajectory[state][1] > Trajectory[state + 1][1] or Trajectory[state][1] < \
                                Trajectory[state + 1][1]:
                        ...
                        elif Trajectory[state][0] == Trajectory[state + 1][0] and Trajectory[state][1] == \
                                Trajectory[state + 1][1]:
                            agent.w_move(0)
...
def plot_timesteps(eps, timesteps2):
    episodes = []
    ...
    #plt.plot(episodes, np.mean(np.asarray(timesteps1), axis=0), label="small-grid: gamma = 1, e = 0.05", color="blue")
    plt.plot(episodes, np.mean(np.asarray(timesteps2), axis=0), label="large-grid: gamma = 1, e = 0.05", color="red")
    plt.xlabel("Episodes")
    plt.ylabel("Timesteps")
    plt.title("Double Q-Learning: Timesteps per Episode Averaged Over 50 Runs")
    plt.legend()
    plt.show()
...
def plot_rewards(eps, rewards2):
    ...
    plt.figure(figsize=(10, 8))
    #plt.plot(episodes, np.mean(np.asarray(cum_rewards1), axis=0), label="small-grid: gamma = 1, e = 0.05", color="blue")
    plt.plot(episodes, np.mean(np.asarray(rewards2), axis=0), label="large-grid: gamma = 1, e = 0.05", color="red")
    plt.xlabel("Episodes")
    plt.ylabel("Cummulative Rewards")
    plt.title("Double Q-Learning: Total Rewards per Episode Averaged Over 50 Runs")
    plt.legend()
    plt.show()

if __name__ == "__main__":
    q, traj, timesteps, rewards = dq_runs(1000, 1)
